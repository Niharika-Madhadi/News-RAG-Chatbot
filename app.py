...
### ====> SIDEBAR <=====
# st.sidebar.title("📰 News RAG Chat Assistant")
# uploaded_file = st.sidebar.file_uploader("Upload Excel or CSV", type=["xlsx", "xls", "csv"])

# if news_metadata:
#     st.sidebar.markdown(f"**Total Articles in Index:** {len(news_metadata)}")
#
# if uploaded_file:
#     df = load_data(uploaded_file)
#     st.success("File loaded!")
#
#     if st.sidebar.button("Train Assistant"):
...
#         st.success("Assistant updated! You can start chatting.")
#         st.rerun()
#
# if st.sidebar.button("Reset Index"):
#     reset_index()
#     faiss_index, news_metadata = None, None
#     st.success("Index reset. Upload and retrain.")
#     st.rerun()
